...
def histograma(df_ur):
    ...
    df_ur.drop(columns=['UTCDateTime','Setpoint_AG','FimDeSemana','HorarioComercial']).hist(figsize=(10, 8), bins=40, edgecolor='black')
    plt.tight_layout()
    plt.show()
...
def dispercao(df_ur):
    ...
    sns.pairplot(df_ur.drop(columns=['UTCDateTime','Setpoint_AG','FimDeSemana','HorarioComercial']))
    plt.show()
...
def boxplot(df):
    ...
    df = df.drop(columns=['UTCDateTime', 'Setpoint_AG','HorarioComercial','FimDeSemana'], errors='ignore')
